...
def solve_routing_problem_optimal(markets, cx, cy, vc, fc, capacity):
    ...
    print("number of markets: ", n_to_refurbish)
...
def solve_routing_problem_heuristic(markets, cx, cy, vc, fc, capacity):
    ...
    c = np.zeros((n + 1,n + 1), dtype=[("cost", int)])
    ...
    prob = VehicleRoutingProblem(G, load_capacity=capacity, num_vehicles=t)
    prob.solve()
    
    refurbishing_cost = prob.best_value + t * fc
    trucks_active_arcs = []
    trucks_path_dicts = []
    ...
    print(trucks_active_arcs)
    print(trucks_path_dicts)
...
def main():
    for i in range(2):
        ...
        refurbishing_cost, t, trucks_active_arcs, trucks_path_dicts = solve_routing_problem_heuristic(markets, cx_markets, cy_markets, vc, fc, capacity)
